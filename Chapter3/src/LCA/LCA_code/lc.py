...
def run_lca(input_path, bnc_all_filtered_path):
    ...
    noundict = {}
    worddict = {}
    wordlistfile = open(bnc_all_filtered_path, "r")
        ...
        for lemma in lemmas:
            ...
            if (not pos in string.punctuation) and pos != "sent" and pos != "sym":
                ...
                if pos[0] == "n":
                    ...
                    (word in adjdict) or (word[-2:] == "ly" and (word[:-2] in adjdict))
                ):
                    lextypes[word] = 1
                    advtypes[word] = 1
                    lextokens += 1
    ...
    uber = (log(wordtokens, 10) * log(wordtokens, 10)) / log(
        wordtokens / float(len(wordtypes.keys())), 10
    )
    ...
    output_dict = dict(zip(key_list, value_list))
    return output_dict
